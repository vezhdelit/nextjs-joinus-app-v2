...
const LeftSidebar = (props: Props) => {
  ...
  return (
    <Box className=" flex h-full w-[240px]">
      <div className=" flex h-full w-full flex-col justify-between overflow-auto rounded-lg bg-white py-10">
          ...
          {sidebarLinks.map((item) => {
            ...
            return (
              ...
              >
                ...
                <h3 className=" pt-[2px] text-base">{item.label}</h3>
              </Link>
            );
          })}
        </div>
        ...
        <form className="flex w-full" action="/api/auth/signout" method="post">
          <button className="flex w-full items-center gap-4 px-12 py-4 hover:bg-sky-50/50 hover:font-medium hover:text-sky-500">
            <TbLayoutSidebarLeftCollapseFilled className=" h-6 w-6" />
            <h3 className=" text-base">Log out</h3>
          </button>
        </form>
      </div>
    </Box>
